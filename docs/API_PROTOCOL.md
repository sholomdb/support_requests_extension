...
- budget source: `view:p298#-#p298:e424:ft_value` = `a3V…` → response gives remaining `e430`,
  the request-record id `pageParam…24397142…` = `a0R…`, program `0123z…`, etc.
...
- Exact origin of the request-record id `a0R…` / `a12…` (appears in home + push state) — is it
  created by "+ רשומה חדשה", or derived in a lookup? Need to record starting from that click.
